...
const { stepReporting: reportingService, lecture: lectureService, concept: conceptService } =
  inject("services");
...
const newuser = computed( () => lecturesInProgress.value.length === 0 && similarLectures.value.length === 0 && connectedConcepts.value.length === 0)
onMounted(async () => {
  ...
  data.forEach(async (report) => {
    ...
    lecture.concepts.forEach(async ({concept}) => {
      ...
      fullConcept.lectures.forEach(({lecture}) => {
        if (!lecture) return;
        if (lecture.id !== report.lectureId && !similarLectures.value.find(({id}) => id === lecture.id)){
